...
function SSRLmicro_check_file(file)
	variable file
	fsetpos file, 0
	string line = ""
	line = myreadline(file)
	if(strsearch(line, "SSRL   MicroEXAFS Data Collector ",0) !=0)
		fsetpos file, 0
		return -1
	endif
	line = myreadline(file)
...
function SSRLmicro_load_data_info(importloader)
	struct importloader &importloader
	importloader.name = "SSRL µEXAFS data"
	importloader.filestr = "*.dat"
	importloader.category = "XAS"
end


function SSRLmicro_load_data([optfile])
	variable optfile
	optfile = paramIsDefault(optfile) ? -1 : optfile
	struct importloader importloader
	SSRLmicro_load_data_info(importloader)
	...
	variable version = str2num(tmps[strlen("SSRL   MicroEXAFS Data Collector "),inf])
	tmps = myreadline(file)
	header += "\rDate: "+stripstrfirstlastspaces(tmps)
	tmps = splitintolist(splitintolist(myreadline(file), "\t"), " ")
	...
	tmps = myreadline(file) // SSRL BEAMLINE
	tmps = myreadline(file) // comments
	...
	for(i=0;i<cols;i+=1)
			...
			if((cols-itemsinlist(offsets,"_"))<=i)
				note $tmps, "Weight: "+stringfromlist(i-(cols-itemsinlist(offsets,"_")),weights,"_")
				note $tmps, "Offset: "+stringfromlist(i-(cols-itemsinlist(offsets,"_")),offsets,"_")
			endif
	endfor	
