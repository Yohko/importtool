...
function SamView_check_file(file)
	variable file
	fsetpos file, 0
	string tmps = mycleanupstr(myreadline(file))
	if(strsearch(tmps, "# Averaged Data",0)!=0 && strsearch(tmps, "created by SamView",0) == -1)
		fsetpos file, 0
		return -1	
	endif
	fsetpos file, 0
...
function SamView_load_data_info(importloader)
	struct importloader &importloader
	importloader.name = "SamView"
	importloader.filestr = "*.avg"
	importloader.category = "XAS"
end


function SamView_load_data([optfile])
	variable optfile
	optfile = paramIsDefault(optfile) ? -1 : optfile
	struct importloader importloader
	SamView_load_data_info(importloader)
